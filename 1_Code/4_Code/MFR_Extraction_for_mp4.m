...
% Input: 
%       reVideo: file path of the recording video in .mp4 format
...
reVideo = 'C:\Work\Code\5_MFR_Examples_v1\2_Example_Video_MP4_Format\Example_recording_video.mp4';   % MODIFY THIS LINE!!!
% file path of the mask file
maskFile = 'C:\Work\Code\5_MFR_Examples_v1\2_Example_Video_MP4_Format\Example_recording_video_mask.mat';   % MODIFY THIS LINE!!!
...
matfile = regexprep(reVideo,'.mp4','_data.mat');
...
if ~isequal(exist(matfile, 'file'), 2)
    ...
    nFrames = v.NumFrames; % number of frames in video stream
    FL = v.FrameRate; % number of video frames per second
   
    % read the first frame and convert the image into greyscale
    aFrame = im2gray(v.readFrame); 
    ...
    save(matfile, 'FL', 'LMag', 'Lfilter','Lflat','nRois', 'regionName');
end

%%
load(matfile, 'FL', 'LMag', 'Lfilter','Lflat','nRois', 'regionName');

% preview the dF/F data 
time = (1:length(Lfilter))/25/60;
for ii = 1:nRois
    figure;
    plot(time,Lfilter(ii,:))
    title(regionName{ii})
    xlabel('Time (min)')
    ylabel('F')
end
